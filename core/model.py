...
class net:
    ...
    def train(self, X_train, X_train_ctx, y_train, regression, loss, n_epochs = 100,
        ...
        if context==True:
            ...
            model.fit([X_train,X_train_ctx], y_train_normalized, batch_size=batch_size, epochs=n_epochs, verbose=1, validation_split=1/num_folds, callbacks=[early_stopping, model_checkpoint, lr_reducer])
        else:
            if regression:
                outputs = Dense(y_train_normalized.shape[1], )(inter)
            else:
            ...
            # We iterate the learning process
            start_time = time.time()
            model.fit(X_train, y_train_normalized, batch_size=batch_size, epochs=n_epochs, verbose=1, validation_split=1/num_folds, callbacks=[early_stopping, model_checkpoint, lr_reducer])
