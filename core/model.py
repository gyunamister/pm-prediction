...
class net:
  ...
  def evaluate(self,x_test, y_test,exp_info):
    ...
    results = self.model.evaluate(x_test, y_test, batch_size=128,callbacks=[csv_logger])
    print("test loss, test acc:", results)

  def train(self, X_train, y_train, regression, loss, n_epochs = 100,
      normalize = False, y_normalize=False, tau = 1.0, dropout = 0.05, batch_size= 128, context=True, num_folds=10, model_name='predictor', checkpoint_dir='./checkpoints/',X_train_ctx=None):
      ...
      if context==True:
          ...
          model.fit([X_train,X_train_ctx], y_train_normalized, batch_size=batch_size, epochs=n_epochs, verbose=1, validation_split=val_split, callbacks=[early_stopping, model_checkpoint, lr_reducer])
      else:
          if regression:
              outputs = Dense(y_train_normalized.shape[1], )(inter)
          else:
          ...
          # We iterate the learning process
          start_time = time.time()
          model.fit(X_train, y_train_normalized, batch_size=batch_size, epochs=n_epochs, verbose=1, validation_split=val_split, callbacks=[early_stopping, model_checkpoint, lr_reducer])
